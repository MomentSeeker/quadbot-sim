...
2. 下蹲 = 4腿 knee:extended；跳跃 = 下蹲后 knee:retracted + hip:backward
3. 抬腿 = 该腿 knee:retracted；伸腿 = 该腿 knee:extended
4. 序列最后一步必须是所有腿 neutral，duration=500（安全归位）
5. 简单动作3-6步，复杂动作8-15步
...
steps:
  1. {duration:300, FR: hip:backward_max, knee:retracted_max}  ← 抬右前腿
  2. {duration:300, FR:backward_max+retracted_max, FL:forward_max+retracted_max}  ← 换左前
  3. {duration:300, 前两腿保持, BL: hip:forward_max, knee:retracted_max}  ← 加左后
  4. {duration:300, 四腿全收, BR: hip:forward_max, knee:retracted_max}  ← 加右后
  5. {duration:500, 所有腿: neutral}

**4. 害怕缩头 (scared) — 下蹲后弹跳**
steps:
  1. {duration:600, FR:hip:backward, FL:hip:forward, 4腿 knee:extended}  ← 下蹲蜷缩
  2. {duration:1000, FR:hip:backward+knee:retracted_max, FL:hip:forward+knee:retracted_max, BR:hip:forward+knee:extended_max, BL:hip:backward+knee:extended_max}  ← 弹跳
  3. {duration:500, 所有腿: neutral}

**5. 青蛙跳 (frog_jump) — 蹲→后腿蹬→前腿蹬→收腿**
steps:
  1. {duration:400, 所有腿 knee:extended}  ← 下蹲蓄力
  2. {duration:100, FR:hip:forward, FL:hip:forward, BR:hip:backward+knee:retracted, BL:hip:backward+knee:retracted}  ← 后腿蹬地
  3. {duration:150, FR:hip:backward+knee:retracted, FL:hip:backward+knee:retracted, BR:hip:backward+knee:retracted, BL:hip:backward+knee:retracted}  ← 全身腾空
  ...
  5. {duration:200, 所有腿 knee:extended}  ← 落地缓冲
  (重复1-5共3次)
  6. {duration:500, 所有腿: neutral}

**6. 趴下/隐藏 (hide) — 所有腿平展贴地**
steps:
  1. {duration:800, FR:hip:forward_max+knee:retracted_max, FL:hip:backward_max+knee:retracted_max, BR:hip:backward_max+knee:retracted_max, BL:hip:forward_max+knee:retracted_max}  ← 趴地缩腿
  2. {duration:500, 所有腿: neutral}

**7. 俯卧撑 (push_up) — 前腿反复收伸**
steps:
  1. {duration:500, 所有腿 knee:neutral}  ← 准备
  2. {duration:400, FR:knee:retracted, FL:knee:retracted, BR:knee:neutral, BL:knee:neutral}  ← 前身抬高
  3. {duration:400, FR:knee:extended, FL:knee:extended, BR:knee:neutral, BL:knee:neutral}  ← 前身下压
  (重复2-3共3次)
  4. {duration:500, 所有腿: neutral}

**8. 起立下蹲 (up_down) — 全身高低波动**
steps:
  1. {duration:500, 所有腿 knee:extended}  ← 蹲下
  2. {duration:500, 所有腿 knee:retracted}  ← 起立
  (重复2次)
  3. {duration:500, 所有腿: neutral}
